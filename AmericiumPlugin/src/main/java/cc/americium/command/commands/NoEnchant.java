...
public class NoEnchant implements Command, Listener {
    private final ArrayList<UUID> toggled = new ArrayList<>();
    private boolean allPrevent = false;

    @Override
    public String[] getCommands() {
        return new String[]{"noenchant"};
    }

    @Override
    public String getDescription() {
        return "Prevent a player or all players from enchanting.";
    }
    ...
    public void execute(Player executor, Sender sender, Context context) {
        ...
        if (args.length == 0) {
            sender.sendMessage(Colors.Prefix + Colors.Failure + "noenchant <player | *>");
            return;
        }
        ...
        if (args[0].equals("*")) {
            ...
            if (!allPrevent) {
                sender.sendMessage(Colors.Prefix + Colors.Failure + "All players can now enchant!");
            } else {
                sender.sendMessage(Colors.Prefix + Colors.Success + "All players can't enchant!");
            }
        ...
        if (toggled.contains(selected.getUniqueId())) {
            ...
            sender.sendMessage(Colors.Prefix + Colors.Failure + selected.getName() + " can now enchant!");
            return;
        }

        toggled.add(selected.getUniqueId());
        sender.sendMessage(Colors.Prefix + Colors.Success + selected.getName() + " can't enchant!");
    }
    ...
    private void onCraft(EnchantItemEvent e) {
        if (this.allPrevent) {
            e.setCancelled(true);
            return;
        }
